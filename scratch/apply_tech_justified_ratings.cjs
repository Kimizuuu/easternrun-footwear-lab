...
// Map of shoe ID -> justified overallRating based on price tier & tech specs
...
// Process content shoe blocks
const shoesBlocks = content.split("  {\n    id: '");

let updatedContent = shoesBlocks[0];

for (let i = 1; i < shoesBlocks.length; i++) {
  let block = "  {\n    id: '" + shoesBlocks[i];
  const idMatch = block.match(/id:\s*'([^']+)'/);
  if (idMatch && idMatch[1]) {
    const id = idMatch[1];
    if (justifiedRatings[id]) {
      const newRating = justifiedRatings[id];
      block = block.replace(/overallRating:\s*\d+/, `overallRating: ${newRating}`);
    }
  }
  updatedContent += (i === 1 ? block.substring(4) : block);
}

fs.writeFileSync(file, updatedContent, 'utf8');
console.log('✅ Applied technology and price justified overall ratings across all Chinese shoes!');
