...
while ((match = shoeRegex.exec(shoesContent)) !== null) {
  ...
  // Clean slug matches s.id or name without double-brand repetition
  const cleanIdSlug = toCleanSlug(id);
  
  shoes.push({ id, name, brand, slug: cleanIdSlug });
}

console.log(`Extracted ${shoes.length} clean shoes with matching ID slugs.`);
...
// 1. Individual Shoe Pages (Clean IDs that resolve in React Router)
shoes.forEach(shoe => {
  sitemap += `  <url>
...
console.log(`✅ Successfully generated exact sitemap.xml with ${1 + shoes.length + selectedComparisons.length} clean, router-resolvable URLs!`);
